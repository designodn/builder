...
- feat: B.2 эпика #350 — sub-agent `text-collector` (`.claude/agents/text-collector.md`) для Шага 6 E.0.5. Вынос ~49 LOC inline recursive walk по closure плана + извлечение `textProps`/`textNode` + определение реального текста (priority `brief > cjm`) из main convo Builder'а в self-contained агента. Shape `text_picks[]` (slug, path, textProp|null, textNode: bool, text, source, ts) не меняется — downstream G-I2-guard / use_figma helper потребляют тот же контракт. Recursive walk строго по `rule_bundle.rulesBySlug` (от bundler'а Шага 6 E.0, переиспользует кэш). Если текст не найден ни в brief, ни в cjm — запись НЕ создаётся (G-I2-guard ловит как `divergence_step: "forgotten_text"`). `designer_override` остаётся отдельным механизмом в main convo (upsert при правке дизайнера в E.1 / drill-down), не от агента. Stateless: Builder сбрасывает `_session.text_picks = []` перед каждым вызовом. Builder.md Шаг 6 E.0.5 сократился с ~49 строк recursive logic до ~30 строк dispatch'а + контракт. AGENT_ARCHITECTURE.md и AGENT_ROLES.md обновлены (диаграмма Шага 6 + listing + новая секция 1.7). Color: не присвоен (как ascii-mockup / architect / slot-reasoner). Закрывает B.2 из roadmap'а #350.
