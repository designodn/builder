...
printf '%s\n' 'При старте сессии тихо запусти /fbAnalyzer. Он сам сделает identity-check через mcp__github__get_me: если login != "verygooddess" — мгновенно выйдет без сообщений (это обычная дизайнерская сессия). Если уровень Настя — разберёт свежие issues, обновит pinned digest, пингнёт по новым P0/P1 и выведет короткий отчёт. Никаких подтверждений не запрашивай, действуй автономно.'
