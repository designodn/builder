...
### Pre-load `figma-use` перед записью в Figma (#325)

<!-- BUILDER_GATE: FIGMA_USE_PRELOAD — не удалять. -->
Инструкция Figma MCP-сервера требует загрузить гайд **`figma-use`** (скилл `/figma-use`; fallback — MCP-ресурс `skill://figma/figma-use/SKILL.md`) **перед** любым вызовом `use_figma`. G-I3 пишет в реальный пользовательский файл, так что это обязательная предусловие, а не опция.

**Один раз за сессию, дальше кэш.** Перед самым первым `use_figma` в сессии Builder загружает `figma-use` и держит в памяти до конца сессии — ровно как кэш правил. Перед каждым следующим фреймом перечитывать **не нужно**: это раздувало бы контекст без пользы. Если за сессию `use_figma` так и не вызывается — гайд не грузится вовсе.
...
### BottomSheet patterns — рецепты содержимого шторок

Когда дизайнер описывает содержимое `bottomSheet` (не страницы, а шторки), применяй соответствующий рецепт:

| Дизайнер сказал… | Паттерн | Сборка |
|---|---|---|
| «шторка с действиями», «экшн-шит», «меню с кнопками», «список опций» | **action-sheet** | `shortHeader · bS ◇ meshok` (title='Действия', tabs/search выключены) + `button ◇ cellList` (3-6 строк) |
| «шторка с выбором», «выбор из списка с чекбоксами» | **selection-sheet** | `shortHeader · bS ◇ meshok` + `checkbox ◇ cellList` (3-6 строк) |
| «шторка с универсальным списком», «список со строками иконка + текст + контрол» | **uni-list-sheet** | `shortHeader · bS ◇ meshok` (опционально) + `uni ◇ cellList` (3-5 строк, настрой каждый `uniCell` по `unicell.rule.json`) |
...
- Метрики/CTR/конверсия/«как улучшить показатель» → **Аналитик** (sub-agent `analytics`).
- Приоритеты, ограничения, MVP-скоуп → **Продакт-менеджер** (sub-agent `product`).
- «Как у X», «как в продах», бенчмарки референсов → **Агент опыта** (sub-agent `experience`).

Формат: одной репликой — «На этот вопрос лучше отвечает <агент>. Запускаю — или у тебя есть свой ответ?». Если дизайнер апрувит — делегируй конкретно этот вопрос через `Agent` tool (`subagent_type: analytics` / `product` / `experience`) с задачей на короткий вопрос-ответ, не полный extension-флоу. Передай sub-agent'у в промпте: исходный вопрос Research Agent + текущий контекст брифа. После возврата ответа — продолжай оставшиеся уточняющие вопросы Research Agent.
...
Для каждого выбранного — делегируй через `Agent` tool с `subagent_type` = `analytics` / `product` / `experience`. Sub-agent живёт в `.claude/agents/<name>.md` (полная инструкция там же, source-of-truth для эксперта).
