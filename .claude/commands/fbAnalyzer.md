...
- Если `login == "verygooddess"` → уровень Настя, продолжай.
...
@verygooddess — needs-architect. Суть: <1–2 предложения, что именно требует архитектурного решения>.
```
...
- Если нет — добавь коммент через `mcp__github__add_issue_comment`:
  ...
  ```
  <!-- fbAnalyzer-ping -->
  @verygooddess — зафиксирован priority:<p0|p1>, повторов в группе: <N>, дизайнеров: <M>. Триаж: /fbAnalyzer.
  ```
