...
**Когда стопать:** очередь пуста | Настя написала «стоп» / «потом» / «достаточно» | встречен компонент уже обработанный в сессии (цикл) | оставшийся бюджет сессии < 25k токенов (минимум на один компонент) | дочерний компонент завершился с `verdict=stuck` или `overbudget` → стоп с предупреждением, не продолжать молча.
