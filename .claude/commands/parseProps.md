...
🔴 **СТОП-ГЕЙТ (#317 рецидив).** Шаги 5–7 **нельзя** начинать, пока не выполнено ОДНО из:
- задан `AskUserQuestion` Шага 4.6 и получен ответ, **либо**
- у правила уже стоит `nestedAsked` (на этот компонент диалог был раньше).

Это не prose-пожелание — машинный якорь с зубами: completeness Шага 7 смотрит на `nestedAsked` и дату файла. **Новое правило** (создано после merge #328) без `nestedAsked` → `pass:false`, тест не закрывается. **Legacy-правило** (создано до merge #328) → `⏳ legacy` в чеклисте, `pass:true` (grace period, самоустраняется при следующем `/parseProps`). Пустая `_nestedQueue[]` больше **не** означает «все атомы» — различает «честно спросили, всё атомы» vs «забыли спросить» только `nestedAsked`.

**Сразу после ответа на `AskUserQuestion` — зафиксируй факт вопроса (ОБЯЗАТЕЛЬНО, единственная команда, обе ветки):**
```bash
node tests/scripts/parseProps-apply-figma.js "<X>" --mark-nested-asked
```
Это привязано к **акту вопроса**, не к результату: зови и при «парсить глубже», и при «все атомы», **до** `--close-nested` и Шага 5. `--close-nested` штамп НЕ ставит (он создаёт стабы и в batch без диалога — ложный сигнал, ревью #328). Если забыл — completeness Шага 7 напечатает ⚠️ с именами слотов.
...
2. Зафиксируй ответ в `nestedProps.policy` родительского правила — не переспрашивать на повторных прогонах. (`policy` — «как обходить вложенный», НЕ сигнал «спросили». Сигнал «спросили» — top-level `nestedAsked`.)
