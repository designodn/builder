...
**Follow-through — авто-очередь (`_nestedQueue[]`):**
...
3. **После завершения текущего компонента** (коммит Шага 6 + чеклист Шага 7) — **немедленно** бери следующий из `_nestedQueue[]` и запускай Шаг 0 для него в текущей сессии. Не ждать новой команды, не останавливаться.
4. **Когда стопать:** очередь пуста | Настя написала «стоп» / «потом» / «достаточно» | встречен компонент уже в очереди (цикл) | суммарный бюджет сессии >100k токенов.
