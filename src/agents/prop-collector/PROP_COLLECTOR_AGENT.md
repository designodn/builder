...
> **Pre-load `figma-use` (#325).** Перед первым `use_figma` в сессии агента загрузи гайд **`figma-use`** (скилл `/figma-use`; fallback `skill://figma/figma-use/SKILL.md`) — инструкция Figma MCP-сервера требует это перед любым `use_figma`, включая probe-чтение. Один раз за сессию агента, дальше кэш.
